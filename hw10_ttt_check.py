...
# 1.0 convert to use cargrdlib
# 1.1 get point values correct
# 1.2 adjust brevity requirements
# 1.3 add vector error test cases
# 1.4 out of order vector moves
# 1.6 use MaxPoints
# 2.0 fix grading
...
class TTTAnalyzerTestCase(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.Points = {"all":15,"tally":5,"style":10,
               'tttr_string':40,'tttr_vector':20,"brevity":10}
        ...
        cls.refcode = {'lines':100,'words':400}
        cls.msgs=[]
        ...
        with open(new_source_file,'w') as f:
            f.write(first_part)
        cls.code_metrics = curl_grading.code_analysis_cpp(new_source_file,cls.lintoptions)
    ...
    test_includes = curl_grading.test_includes
    ...
    def test_brevity(self):
        ...
        self.Points['brevity'] = min(13, max(0,10- (lines_words- target)/50))
    ...
    def test_tttstring(self):
      ...
      self.process = sub.Popen([self.baseprogname],**popen_specs)
      ...
      for board in Boards:
          try:
              ...
              if board in MapAnswer:
                ...
                if ans in 'xot':
                  errors.append(f"problem with case {board}\n correct: 'i or c'\n   yours: {ans!r}\n")
          except BrokenPipeError:
              errors.append(f"your program previously crashed.\nproblem with case {board!r}\n")
      ...
      if not errors and summary != "aeb564dd31a04154eff23ebba7661b427b8389c18259b626cb6ce434c4811e46":
        ...
        basescore = 75
        base_report += f"BASESCORE = {basescore:.2f}, some i /c errors detected\n"
      ...
      for board in StringErrors:
          try: 
            ans = ask_program(self.process,board,"s")
            if ans=="":
                error_errors.append("THIS IS LIKELY WHERE YOUR PROGRAM CRASHED.")
            if ans != "e":
              error_errors.append(f"problem with case {board!r}\n correct: 'e'\n   yours: {ans!r}\n")

          except BrokenPipeError:
              error_errors.append(f"your program previously crashed.\nproblem with case {board!r}\n")
            
      if error_errors:
          basescore = max(0,basescore - 5*len(error_errors))
          base_report += f"BASESCORE adjusted (due to error handling problems) to {basescore:.2f}\n"

      errors = error_errors + errors

      if len(errors)>30:
         errors=errors[:30]
         errors.append('...only reporting the first 30 errors.\n\n')

      #self.msgs.extend(errors)
      #self.msgs.append(base_report)
      self.Points['tttr_string']=5+ .35*basescore
      shutdown_program(self)
    ...
    def test_tttvector(self):
      ...
      for board,res in vector_games.items():
        with self.subTest(CASE=f"{board}"):
          try:
            ans = ask_program(self.process,board,"v")
            if ans == res:
              continue
            self.Points['tttr_vector'] -= 5
            if ans != res:
                self.fail(f"problem with vector case:\n{board}\n correct: {res!r}\n   yours: {ans!r}\n")
          except BrokenPipeError:
            self.fail("crashing!!\nproblem with case {}\n".format(repr(board)))
      
      if self.Points['tttr_vector']<0:
        self.Points['tttr_vector']=0
      shutdown_program(self)
    ...
    def test_get_all_boards(self):
        ...
        if summary != "26fd02c0d9e0504402322578e85aa24d5fa0b520ce92baecb750b8853553bac5":
            self.fullFail("all",'you are not generating the boards correctly.\n')
    ...
    def test_tally(self):
        ...
        if summary != "0a04c8dacc8b13688bebdd246dda943567c006f1eecad9c205dc2208db585a28":
            self.fullFail("tally", 'your tally of results is not correct.\n')
        tally={}
        ...
        for board in Boards:
            try:
                ...
                pass
        if tally != the_tally:
            self.fullFail("tally",'your tally seems to be fake.')
        shutdown_program(self)
...
if __name__ == '__main__':
    print('TTT Analyzer (HW10) Checker Version {0}.{1}\n'.format(*VERSION))
    g={}
